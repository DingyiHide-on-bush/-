...
import efforts
from Ui_fig_press import Ui_Form
import numpy as np
...
class Form(QDialog, Ui_Form):
    ...
    def on_pushButton_clicked(self):
        ...
        if ax=='' or ay=='' or wn=='' or kn=='':
            ...
            if self.check(ax, ay, wn, kn, degree)==1:
                ...
                ay[-2]=ay[-1]+self.k2*(ax[-2]-ax[-1])
                #校正model
                ay0=ay[1:-1]
                self.model['压力面控制点纵坐标']='，'.join(list(map(str, ay0)))
    ...
    def on_pushButton_3_clicked(self):
        #还原
        self.text_x.setText(self.model['压力面控制点横坐标'])
        self.text_y.setText(self.model['压力面控制点纵坐标'])
        self.text_w.setText(self.model['压力面控制点权重'])
        self.text_k.setText(self.model['压力面节点矢量'])
        self.spinBox.setValue(int(self.model['压力面阶数']))
    ...
    def on_pushButton_4_clicked(self):
        #导出
        dic_mod={'压力面控制点横坐标':self.text_x.toPlainText(), \
                '压力面控制点纵坐标':self.text_y.toPlainText(),\
                '压力面控制点权重':self.text_w.toPlainText(),\
                '压力面节点矢量':self.text_k.toPlainText(),\
                '压力面阶数':str(self.spinBox.value())}
